...
#         Use the 'Find_Excel_and_CSV_Files' tool to list all relevant files. Once the file is identified,
...
def create_chart(input_string: str) -> str:
    ...
    try:
        ...
        file_path = os.path.join(BASE_DIR, parts[0].strip())  # Use strip to remove whitespace
        # Create a list of column names, stripping whitespace from each
        column_names = [name.strip() for name in parts[1].split(',')]  # Strip whitespace from column names
        ...
        file_name = f"{chart_type.lower().replace(' ', '_')}_{uuid.uuid4()}.png"  # Use chart type in file name
        plt.savefig(file_name)
        plt.close()
        return file_name
    except Exception as e:
...
def list_files(directory: str) -> str:
    ...
    If an error occurs (e.g., directory not found), it returns an error message specifying the issue.
    """
    full_directory_path = os.path.join(BASE_DIR, directory)  # Ensure full path is constructed with BASE_DIR
    try:
...
def get_column_names(file_path: str) -> str:
    ...
    If an error occurs during file reading (e.g., file is corrupted or unreadable), it provides an error message detailing the problem.
    """
    full_file_path = os.path.join(BASE_DIR, file_path.strip())  # Ensure path is clean and relative to BASE_DIR
    print(f"Attempting to access file at: {full_file_path}")  # Debugging output
